...
export const homeInput = z.object({
  ...
  constructionYear: z.number().int().min(1200).max(new Date().getFullYear() + 2).optional(),
...
export async function createHome(userId: string, raw: unknown) {
  ...
  return db.transaction(async (tx) => {
    const [home] = await tx.insert(homes).values({ ...input, ownerId: userId }).returning();
    if (!home) throw new AppError("INTERNAL_ERROR", "Could not create the home.", 500);
    await tx.insert(homeMembers).values({ homeId: home.id, userId, role: "OWNER", invitedBy: userId });
    return home;
  });
}
...
export async function listHomes(userId: string) {
  return db
    .select({ id: homes.id, name: homes.name, type: homes.type, city: homes.city, role: homeMembers.role })
    .from(homeMembers)
