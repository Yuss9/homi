    ...
    <main id="main" className="app-main">
      <div className="dashboard-head">
        <div>
          ...
          <h1>Repairs</h1>
          <p>
            Track the full issue, provider, cost, completion date, and warranty
            claim.
          </p>
        </div>
        <select
      ...
      <div className="dash-grid" style={{ marginTop: 32 }}>
        <section className="dash-card">
            ...
            {repairs.map((repair) =>
              editingRepairId === repair.id ? (
                <form
                  ...
                  key={repair.id}
                  onSubmit={(event) => void saveRepair(event, repair)}
                >
                  <div className="dash-card-head">
                    <h2>Edit repair</h2>
                    <button
                  ...
                  <div className="inline-actions">
                    ...
                    <button
                      ...
                      type="button"
                      onClick={() => void archiveRepair(repair)}
                    >
                      <Archive size={15} />
                      Archive
                    </button>
                ...
                <div className="dash-task has-action" key={repair.id}>
                  ...
                  <div>
                    ...
                    <small>
                      {repair.provider || "No provider"} · {repair.issueDate}
                      {repair.cost
                        ? ` · ${repair.cost} ${repair.currency ?? "EUR"}`
                        : ""}
                    </small>
                    {repair.repairDate && (
                      <small>Repaired {repair.repairDate}</small>
                    )}
                  </div>
                  ...
                  <button
                    ...
                    onClick={() => setEditingRepairId(repair.id)}
                  >
                    <Pencil size={16} />
                  </button>
                </div>
          ...
          {!repairs.length && (
            <p className="muted-copy">No repairs have been logged.</p>
          )}
        </section>
        ...
        <form className="dash-card auth-form" onSubmit={addRepair}>
          ...
          <button className="button" disabled={!assets.length} type="submit">
            <Plus size={16} />
            Log repair
          </button>
        </form>
